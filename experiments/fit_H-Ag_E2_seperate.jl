...
Threads.@threads for N in 1:2:20
    println("The number of this iteration is $N, ID is $(Threads.threadid())")
end
...
#basis_dict = Dict()
rcut_factors = [1.5, 2.0, 2.5, 3.0, 3.5]
Threads.@threads for (maxorder,maxdeg) = [(2,4),(2,8),(2,12),(2,14),(3,4),(3,6),(4,4),(4,6),(3,8),(3,10)] 
...
rcut_factors = [1.5, 2.0, 2.5, 3.0, 3.5]
Threads.@threads for (maxorder,maxdeg) = [(2,4),(2,8),(2,12)] 
    rcut = rnn(:Ag)
    r0 = rnn(:Ag)
    Bsel = ACE.SparseBasis(; maxorder=maxorder, p = 2, default_maxdeg = maxdeg ) 
    RnYlm = ACE.Utils.RnYlm_1pbasis(;  r0 = r0, 
    ...
    Zk = ACE.Categorical1pBasis(species; varsym = :mu, idxsym = :mu, label = "Zk")
    B1p = RnYlm * Zk
    if !((maxorder,maxdeg) in basis_dict)
        basis_dict[(maxorder,maxdeg)] = ACE.SymmetricBasis(ACE.EuclideanMatrix(), B1p, Bsel)
    end
    onsite_H = basis_dict[(maxorder,maxdeg)]
