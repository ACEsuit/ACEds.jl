...
using HDF5, JLD
using ACEds.OnsiteFit: get_X_Y, get_onsite_data,array2svector, get_X_Y2, collect_bdata,  get_X_Y3
using DataFrames, CSV
using ACEds.Utils: dMatrix2bMatrix, sparsesub
...
basis = read_dict(load_json(string(path,"/test-max-",3,"maxdeg-",5,".json")))
model = OnSiteModel(basis, cutoff(basis))
zH = AtomicNumber(:H)
train_bdata = collect_bdata(model, zH, train_data; use_chemical_symbol = true)
test_bdata = collect_bdata(model, zH, train_data; use_chemical_symbol = true)
X_train, Y_train = get_X_Y3(train_bdata);
#X_train, Y_train = toMatrix(X_train), toMatrix(Y_train)
X_test, Y_test = get_X_Y3(test_bdata);
#X_test, Y_test = toMatrix(X_test), toMatrix(Y_test)
#coeffs = qr_solve(X_train, Y_train;);
using GLMNet
train_cv = glmnetcv(X_train, Y_train)
coffs  = train_cv
train_error, test_error = ACEds.LinSolvers.rel_error(coeffs, X_train,Y_train), ACEds.LinSolvers.rel_error(coeffs, X_test,Y_test)
#%%                        
for (maxorder,maxdeg) = [(3,5)]
