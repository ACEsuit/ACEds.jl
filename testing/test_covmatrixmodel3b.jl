...
mdata2_onsite =  Dict(
    "train" => [(friction_tensor=reinterpret(Matrix,d.friction_tensor), B =  [reinterpret(Matrix,Matrix(b[d.friction_indices,d.friction_indices])) for b in d.B[get_range(mb,:onsite)]] ) for d in mdata_sparse],
    "test" => [(friction_tensor=reinterpret(Matrix,d.friction_tensor), B =  [reinterpret(Matrix,Matrix(b[d.friction_indices,d.friction_indices])) for b in d.B[get_range(mb,:onsite)]] ) for d in mdata_sparse_test]
)
...
c_matrix_main = σ .* randn(n_rep_main, size(c_matrix_all,2))
c_matrix_onsite = σ .* randn(n_rep_onsite, nparams(mb,:onsite))
...
c_matrix_all[1:n_rep_main,:] = copy(c_matrix_main)
