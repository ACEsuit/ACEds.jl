...
params_ref_vec= [rand(n_basis) for i=1:3]
params_vec= [rand(n_basis) for i=1:3]

Σ = Sigma(params_vec,B)
Γ = Gamma(params_vec,B)

#implement sigma 
Γ = Gamma(params_vec,B)

Γ2 =  Gamma(vcat([p for p in params_vec]...),vcat([b for b in B]))
Γ-Γ2
rank(toMatrix(Γ))
...
train = @showprogress [ 
    begin 
        ...
        (at=deepcopy(at), Γ = Gamma(params_ref_vec, basis)) 
    end 
    for i=1:n_data ];
...
seed!(1234);
params_vec = [deepcopy(params_ref) + .1* rand(n_basis) for params_ref in params_ref_vec]

ps = Flux.params(params_vec)
grads = Flux.gradient(() -> loss_all(params_vec, train_data),Params([params_vec]))
...
#Descent(1E-10)
Base.zero(aa::Vector{Vector{T}}) where {T<:Real}= [zero(a) for a in aa]

n_rep = 3
params= rand(n_rep*n_basis) 

nsteps = 10
loss_traj = [loss_all(params, train_data)]
test_loss_traj = [loss_all(params, test_data)]
#params_loss_traj = [sum(map(x->abs.(x),((params.-params_ref_vec))))]
#params_loss_traj = []
grad_norm_traj = []
@showprogress for i=1:nsteps
    ...
    #push!(params_loss_traj, sum(map(x->abs.(x),params_vec-params_ref_vec)))
end
